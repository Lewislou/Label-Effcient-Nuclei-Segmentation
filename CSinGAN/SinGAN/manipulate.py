...
def generate_gif(Gs,Zs,reals,NoiseAmp,opt,alpha=0.1,beta=0.9,start_scale=2,fps=10):
    ...
    del images_cur
#随机模式
def SinGAN_generate(Gs,Zs,reals,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=1): #设置生成sample的数量
    ...
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        ...
        for i in range(0,num_samples,1):
            ...
            I_curr = G(concat,I_prev)
            #plt.imsave('./heatmap/cell/'+str(n)+'.png', functions.convert_image_np(I_curr.detach()), vmin=0,vmax=1)
            if n == len(reals)-1:
                if opt.mode == 'train':
...
#重建模式
def SinGAN_generate_recon(Gs,Zs,reals,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=1): #设置生成sample的数量
    #if torch.is_tensor(in_s) == False:
    if in_s is None:
        in_s = torch.full(reals[0].shape, 0, device=opt.device)
    images_cur = []
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        ...
        for i in range(0,num_samples,1):
            ...
            z_in = noise_amp*(z_curr)+I_prev
            I_curr = G(z_in.detach(),n,I_prev)
            plt.imsave('./reconheatmap/cell/'+str(n)+'.png', functions.convert_image_np(I_curr.detach()), vmin=0,vmax=1)
            if n == len(reals)-2:
                if opt.mode == 'train':
                    dir2save = '%s/RandomSamples/%s/gen_start_scale=%d' % (opt.out, opt.input_name[:-4], gen_start_scale)
                else:
